...
class ConfidenceConstants:
    ...
    ``cf_norm_trm`` overrides that interpolation with a fixed interval; it is
    ``None`` by default, so nothing here departs from that scheme unless a
    caller asks it to.
    ...
    #: Eq. 19 for CF_trm. ``None``, the default, is the published behaviour:
    #: CF_trm has no interval of its own and is normalized on the day and
    #: night intervals interpolated by the Eq. 20 weight. Setting an interval
    #: here switches CF_trm to it -- a hook for callers who want to try a
    #: terminator scale of their own, not a change to what DEBRA does.
    cf_norm_trm: Bounds | None = None
